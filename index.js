...
function desktopAppHandleDragStart(event) {
    ...
    const move = (event) => {
        ...
        let newDraggedDesktopIconOffsetX = (-((desktop_apps.getBoundingClientRect().left + (newDraggedDesktopIconX * el.clientWidth)) - event.clientX) / el.clientWidth) - 0.5;
        let newDraggedDesktopIconOffsetY = (-((desktop_apps.getBoundingClientRect().top + (newDraggedDesktopIconY * el.clientHeight)) - event.clientY) / el.clientHeight) - 0.5;
        el.style.transform = `translate(${newDraggedDesktopIconOffsetX}%, ${newDraggedDesktopIconOffsetY}%)`;
